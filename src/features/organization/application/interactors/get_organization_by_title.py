...
class GetOrganizationByTitleInteractor:
    def __init__(self, organization_repository: IOrganizationRepository):
        self._repository = organization_repository

    async def execute(self, organization_title: str) -> OrganizationDTO:
        organization = await self._repository.get_organization_by_title(organization_title)

        if not organization:
            raise OrganizationNotFoundError(message=f'Organization with title {organization_title} not found')
